...
export default function FinanceBillingPipeline({ pipeline }) {
  ...
  return (
    <div className="pm-card finance-dash-card finance-billing-pipeline finance-billing-pipeline--sales">
      <div className="finance-dash-card__head finance-billing-pipeline__head">
        <div>
          <span>Sales billing pipeline</span>
          {pipeline?.total_actionable > 0 ? (
            ...
            <p className="finance-billing-pipeline__meta">No pending billing actions</p>
          )}
        </div>
      </div>
      <div className="finance-billing-pipeline__grid finance-billing-pipeline__grid--sales-row">
            ...
            {(queue.samples || []).length > 0 ? (
              <ul className="finance-billing-pipeline__samples">
                {queue.samples.slice(0, 3).map((row) => (
                  <li key={row.name}>
                    <span>{row.name}</span>
                    <span className="finance-cell-muted">{row.customer || "—"}</span>
                    {row.amount != null ? (
