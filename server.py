...
async def stream_agent_turn(text, session_id=None, user_id=None, history=None, task_context=None):
    """The single live agent turn implementation, shared by /api/chat/stream
    and the realtime voice WS path. Keeps its own `history` list
    (caller-owned, per-connection/session) rather than a LangGraph
    checkpointer -- see load_stream_history/save_stream_history below,
    backed by stream_history.sqlite via a bare AsyncSqliteSaver. Yields
    token/tool_call/tool_result/done event dicts in call order.
