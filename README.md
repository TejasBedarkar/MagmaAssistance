...
- **Backend** — a Flask API that runs the agent pipeline: speech/text understanding via **LangChain + Ollama (LLM)**, and spoken replies via **VibeVoice (TTS)**, with **Whisper** available for server-side speech-to-text.
...
Browser mic/text
 ...
 Web Speech API (STT, in-browser)
       │  text
       ▼
 React chat UI  ──POST /api/chat──▶  Flask server
       ▲                                 │
...
- Spoken replies synthesized with VibeVoice and played back in the browser
...
- **Disk space**: reserve at least **3–4 GB free** for the downloaded models — VibeVoice-Realtime alone is roughly **0.5B parameters**, and Whisper adds more on top depending on the size you pick.
- **GPU**: a CUDA-capable GPU is recommended for all three model stages (LLM via Ollama, Whisper STT, and VibeVoice TTS). Inference will fall back to CPU without one, but replies — especially TTS synthesis — will be noticeably slower.
- **This is a local/offline-first setup.** Once the app is deployed on an online server (with more VRAM/compute available), the LLM, STT, and TTS models can each be swapped for larger, higher-quality versions — see the `WHISPER_MODEL` / `LLM_MODEL` / `TTS_SPEAKER` config below.
...
4. Download the Whisper and VibeVoice models via `ModelDownload.py` into `backend/WhisperSTT/model` and `backend/VibeVoiceTTS/models`.
5. Run `npm install` for the frontend.
6. Launch the backend (`python server.py`) and frontend (`npm run dev`) together.
...
python ModelDownload.py --whisper-dir backend\WhisperSTT\model --vibevoice-dir backend\VibeVoiceTTS\models --tool-rag-dir backend\ERP\models\all-MiniLM-L6-v2
...
|---|---|---|---|
| `frontend/.env` | `VITE_API_URL` | `http://localhost:8000` | Backend API base URL |
...
| `backend` (env var) | `TTS_SPEAKER` | `Carter` | VibeVoice speaker preset |
...
3. **Memory Optimization:** Modified `backend/Main.py` to initialize `VibeVoiceTTS` lazily on demand. This reduces startup RAM requirements by **5.4 GB**, enabling direct execution on standard CPU hardware.
