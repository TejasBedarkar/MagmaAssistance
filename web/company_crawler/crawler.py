...
def _allowed_by_robots(url: str) -> bool:
    """Best-effort robots.txt check with domain-level in-memory cache and 1s timeout.
    Fails open if robots.txt can't be fetched or parsed.
    """
    try:
        parsed = urlparse(url)
        ...
        if netloc in _ROBOTS_CACHE:
            ...
            return rp.can_fetch(USER_AGENT, url) if rp is not None else True

        robots_url = f"{parsed.scheme}://{parsed.netloc}/robots.txt"
        rp = None
        try:
            resp = _SESSION.get(robots_url, timeout=1.0)
...
def fetch_rendered_html(url: str) -> str | None:
    ...
    Uses domcontentloaded and a short 4s timeout to avoid getting stuck on tracking beacons.
    """
    try:
...
def find_contact_pages(homepage_url: str, max_pages: int = 3, homepage_html: str | None = None) -> list[dict]:
    """
    ...
    If homepage_html is provided, reuses it without making redundant HTTP requests.
    """
    if homepage_html is not None:
        html_text = homepage_html
    else:
        resp = _get(homepage_url)
        html_text = resp.text if resp is not None and resp.status_code < 400 else ""
...
def get_page_html(url: str) -> tuple[str, str]:
    ...
    if not html or (_looks_js_rendered(html) and not spa_payload):
