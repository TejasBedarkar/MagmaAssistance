...
_ZAUBACORP_CACHE: dict[str, Optional[ZaubaCorpData]] = {}
...
def fetch_zaubacorp_html(url: str) -> tuple[str, str]:
    ...
    """
    try:
        resp = _SESSION.get(url, timeout=4.0, allow_redirects=True, verify=False)
...
def lookup_zaubacorp_fallback(company_name: str, domain: Optional[str] = None) -> Optional[ZaubaCorpData]:
    """
    Main orchestrator function with in-memory caching:
    1. Checks cache for instant lookup.
    2. Finds ZaubaCorp profile URL for company_name.
    3. Fetches and parses the ZaubaCorp page.
    4. Caches and returns structured ZaubaCorpData if found, or None.
    """
    if not company_name:
        return None

    clean_key = _clean_company_query(company_name).lower()
    if clean_key in _ZAUBACORP_CACHE:
        return _ZAUBACORP_CACHE[clean_key]
    if domain and domain.lower() in _ZAUBACORP_CACHE:
        return _ZAUBACORP_CACHE[domain.lower()]

    zauba_url = search_zaubacorp(company_name, domain)
    if not zauba_url:
        logger.info("No ZaubaCorp profile found for '%s'.", company_name)
        _ZAUBACORP_CACHE[clean_key] = None
        if domain:
            _ZAUBACORP_CACHE[domain.lower()] = None
        return None
    ...
    _ZAUBACORP_CACHE[clean_key] = data
    if domain:
        _ZAUBACORP_CACHE[domain.lower()] = data
    return data
