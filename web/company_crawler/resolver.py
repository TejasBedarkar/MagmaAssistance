...
def resolve_company_website(
    ...
    # Verification pass
    if verify_pages and top:
        def _verify_candidate(cand: Candidate):
            try:
                t, d = _fetch_page_meta(f"https://{cand.domain}")
                return cand, t, d
            except Exception:
                return cand, "", ""

        with concurrent.futures.ThreadPoolExecutor(max_workers=min(3, len(top))) as executor:
            futures = [executor.submit(_verify_candidate, c) for c in top]
            for f in concurrent.futures.as_completed(futures):
                c, title, desc = f.result()
                norm_name = _normalize_name(company_name)
