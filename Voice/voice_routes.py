...
Endpoints:
  ...
  POST /api/voice/session/{id}/tool  — Executes an ERP tool (audited, authorized)
...
All tool execution goes through server.py's _execute_tool(), guaranteeing:
  ...
  - ERPIdentity (per-user credentials) enforcement
...
from LLM.LLM import GENERAL_ERP_PROMPT
...
async def get_voice_config(session_id: str, state=Depends(_get_server_state)):
    ...
    the WebRTC data channel opens. Includes:
      - system_prompt (GENERAL_ERP_PROMPT adapted for voice)
    ...
    voice_prompt = (
        "You are Magna, a professional AI voice assistant for a manufacturing ERP system. "
        ...
        "You have access to ERP tools. Always confirm destructive actions before executing them."
    )
...
async def execute_voice_tool(
    ...
    """
    Executes an ERP tool on behalf of the voice session.
    Security checks:
