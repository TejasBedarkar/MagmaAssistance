...
Usage:
    ...
    python magma_cli.py upload-po path/to/po.pdf
...
# Falls back through the several aliases used in .env (ERP_*, ERPNEXT_*,
# FRAPPE_*) so this works regardless of which naming convention is set.
DEFAULT_ERP_API_KEY = (
    os.environ.get("ERP_API_KEY") or os.environ.get("ERPNEXT_API_KEY") or os.environ.get("FRAPPE_API_KEY")
)
DEFAULT_ERP_API_SECRET = (
    os.environ.get("ERP_API_SECRET") or os.environ.get("ERPNEXT_API_SECRET") or os.environ.get("FRAPPE_API_SECRET")
)
...
class MagmaClient:
    ...
    def upload_po(self, filepath: str) -> dict:
        return self._upload("/api/upload-po", filepath)
...
def _handle_command(client: MagmaClient, line: str) -> bool:
    ...
    if cmd == "/help":
        ...
        elif len(parts) == 1 and DEFAULT_ERP_API_KEY and DEFAULT_ERP_API_SECRET:
            api_key, api_secret = DEFAULT_ERP_API_KEY, DEFAULT_ERP_API_SECRET
            print("(using ERP_API_KEY / ERP_API_SECRET from .env)")
        else:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_login = sub.add_parser("login", help="Bind ERPNext credentials to the session (defaults to .env's ERP_API_KEY/SECRET)")
    p_login.add_argument("api_key", nargs="?", default=DEFAULT_ERP_API_KEY)
    p_login.add_argument("api_secret", nargs="?", default=DEFAULT_ERP_API_SECRET)
    ...
    try:
        if args.command == "chat":
            ...
            if not args.api_key or not args.api_secret:
                print("[error] No api_key/api_secret given and none found in .env (ERP_API_KEY/ERP_API_SECRET).")
                sys.exit(1)
