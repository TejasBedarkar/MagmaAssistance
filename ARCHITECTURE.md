...
- [x] **`erp_theme` — identity wiring + WebRTC voice cleanup merged into its own `cleanup/consolidation`** (mirrors the backend's integration branch, `CONTRIBUTING.md §1`). `credentials: 'include'` removed from all 5 fetch calls (backend reads `sid`/`csrf_token` from the request body, never a cookie), `ChatArea.jsx`'s `API_BASE_URL` now auto-detects local vs prod, and `useVoiceSession.js`/`AudioCapture.js` deleted (unused, matches the backend WebRTC deletion). Mergeable to `main` now, functionally — still your call on when to actually unfreeze `main` for it.
