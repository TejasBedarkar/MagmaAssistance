...
- [x] **`erp_theme` frontend branch (`feature/identity-wiring`) — both functional pre-merge blockers fixed (`7c88841`).** `credentials: 'include'` removed from all 5 fetch calls — the backend reads `sid`/`csrf_token` from the request body, never from a cookie, so it bought nothing except a mode where chat breaks entirely if `ALLOWED_ORIGINS` is unset. `ChatArea.jsx`'s `API_BASE_URL` (was hardcoded to `localhost:8050`, a committed local-dev override) now uses the same host-detection fallback as `dev/local`. Mergeable to `main` now, functionally.
- [ ] **Separate, still-open security gap — `ALLOWED_ORIGINS` is about safety, not just function.** Fixing the item above means chat *works* with CORS wide open (`allow_origins=["*"]`, the default) — it does not mean that's safe. `*` lets any website's JS call `/api/chat/stream`, `/api/upload-document`, `/api/session/identify` and read the response; CORS does nothing against a direct server-to-server call either way. If a user's `sid`/`csrf_token` ever leaks (XSS elsewhere, a malicious extension reading `document.cookie`), a page on any origin can replay it against our backend directly. Pre-existing gap (see §1 "No authentication on the AI backend"), not introduced by this branch. No real users yet, so not urgent — but **`ALLOWED_ORIGINS` must be locked down to the real prod origins before this touches real user data**, independent of whether chat still works without it.
