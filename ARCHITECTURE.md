...
- [x] **`erp_theme` frontend branch (`feature/identity-wiring`) — both pre-merge blockers fixed (`7c88841`).** `credentials: 'include'` removed from all 5 fetch calls — the backend reads `sid`/`csrf_token` from the request body, never from a cookie, so it bought nothing except a CORS failure mode when `ALLOWED_ORIGINS` is unset. `ChatArea.jsx`'s `API_BASE_URL` (was hardcoded to `localhost:8050`, a committed local-dev override) now uses the same host-detection fallback as `dev/local`. Still **not merged to `main`** — no real users on this ERP yet, so no urgency, but it's mergeable now.
