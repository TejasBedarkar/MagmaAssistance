...
- [ ] **`erp_theme` frontend branch (`feature/identity-wiring`) not merged to `main` yet.** No real users on this ERP yet — still dev phase — so none of this is urgent, but two things must be fixed before this branch ever ships:
  1. **`credentials: 'include'`** was added to the backend fetch calls (`/api/chat/stream`, `/api/upload-document`, `/api/tts*`) so the browser sends the session cookie cross-origin. Confirmed by simulating the preflight: with `ALLOWED_ORIGINS` unset (today's default), the backend answers `Access-Control-Allow-Origin: *` with no credentials header, and the browser refuses the request outright — **chat stops working entirely for every user**, not a graceful fallback to the service account. **Deploy gate: `ALLOWED_ORIGINS` must be set in EC2 `.env` (`CONTRIBUTING.md §6`) before this branch merges to `main`.**
  2. **`ChatArea.jsx`'s `API_BASE_URL` is hardcoded to `http://localhost:8050`** on this branch — a local-dev override that got committed instead of staying local-only (the same mistake `CONTRIBUTING.md §7` warns about for `dev/local`). Must be reverted to the real prod backend URL before merge, or every visitor's browser tries to call `localhost`.
